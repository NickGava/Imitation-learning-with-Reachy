...
--- Output (single run) ---
  data/dataset/exercise_XXX/n_XX/GRU/bc_model_fold_0.pth  ...

--- Output (multi-run, --run N) ---
  data/dataset/exercise_XXX/n_XX/GRU/run_N/bc_model_fold_0.pth  ...

Usage:
  py -m bc_approach.GRU.train_bc 1
  py -m bc_approach.GRU.train_bc 1 --run 2
'''
...
def _train_fold(df_trn, df_val, device, fold_idx, output_dir):
    ...
    with open(output_dir / f'scaler_fold_{fold_idx}.pkl', 'wb') as f:
    ...
    for epoch in range(1, MAX_EPOCHS + 1):
        ...
        if is_best:
            ...
            torch.save({'model_state': model.state_dict(), 'input_dim': N_INPUT,
                        'output_dim': N_JOINTS, 'hidden_size': HIDDEN_SIZE,
                        'n_layers': N_LAYERS, 'seq_len': SEQ_LEN, 'val_loss': val_loss,
                        'hparams': {'hidden_size': HIDDEN_SIZE, 'n_layers': N_LAYERS,
                                    'seq_len': SEQ_LEN, 'lr': LR, 'batch_size': BATCH_SIZE,
                                    'noise_std': NOISE_STD}}, model_path)
        else:
            patience_count += 1
        if epoch % 10 == 0 or is_best:
            print(f'  {epoch:>5}  {trn_loss:>12.6f}  {val_loss:>12.6f}{"  ←" if is_best else ""}')
...
def _save_loss_curve(all_train, all_val, path):
