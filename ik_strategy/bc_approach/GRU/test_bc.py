...
def run_bc_loop(ensemble, n_steps: int, start_pose: dict) -> np.ndarray:
    ...
    Each timestep in the window contains [q, dq] (16 features).
    ...
    hist_len = seq_len + VELOCITY_LAG  # need extra frames to compute dq

    # Initialise extended history with start pose
    ...
    for step in range(n_steps):
        ...
        # Build (seq_len, 16) window: last seq_len frames with their dq
        window = np.zeros((seq_len, N_INPUT), dtype=np.float32)
        for i in range(seq_len):
            t  = VELOCITY_LAG + i          # index in hist_arr
            q  = hist_arr[t]
